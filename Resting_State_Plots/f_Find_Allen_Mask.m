...
% structure
function [mask,abbreviation] = f_Find_Allen_Mask(Side,ROI,parcellation)
...
for iROI = 1:tmpnROI
    if strcmp(tmpNames(iROI),ROI)
        if strcmp(Side,"Right")
            ...
            abbreviation = char(parcellation.RightValid(iROI));
            break
        elseif strcmp(Side,'Left')
            mask = parcellation.LeftROIs(:,:,iROI);
            abbreviation = char(parcellation.LeftValid(iROI));
            break
        end
    end
end
