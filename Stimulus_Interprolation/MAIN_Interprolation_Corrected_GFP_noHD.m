...
Run = 10;
Version = 2; % This is the version of correction if correction was run several times

% Define File Names
fname.Save = fullfile(root,'mat',sprintf('Run%i_Interporlated_TC_GFP_NoHD_CorrectionV%i.mat',Run,Version));
...
fname.Correction_Settings = fullfile(root,'mat',sprintf('CorrectionSettings_GFPNoHD_Run%i_V%i',Run,Version));
...
if Correction_Settings.gfp.Correct
    gfp_norm=h5read(fname.CorrectedData,'/gfp/norm');
else
    gfp_norm=h5read(fname.UncorrectedData,'/gfp/norm');
end
...
Interp_Parameters.SizeY = size(gfp_norm,1);
Interp_Parameters.SizeX = size(gfp_norm,2);
clear tmp*
%% Apply Brain Mask
for index = 1:size(gfp_norm,3)
...
% GFP
fprintf('\nBeggining Interprolation\n');
GRAB_All_Runs_NoHD = fInterprolation(gfp_norm,tGRAB,'GFP',Interp_Parameters);
fprintf('\nDone with all Interprolation!\nSaving\n');

%% Save Intreprolated data
save(fname.Save,"GRAB_All_Runs_NoHD",'tNew','-v7.3');
